...
# Data processing ----
base_df <- rbind(reference_df, base_df)
...
#     fill_color = case_when(
#       align_state %in% c("REF", "MISMATCH", "SOFT_CLIPPED", "DELETION") ~ config$visual$colors$base[base],
#       align_state == "MATCH" ~ config$visual$colors$base[base],
#       align_state == "INSERTION" ~ config$visual$colors$base[base],
#       TRUE ~ "black"
#     ),
#     border_color = case_when(
#       align_state %in% c("HARD_CLIPPED", "SOFT_CLIPPED", "REF", "DELETION") ~ config$visual$colors$border[align_state],
...
ordered_y <- sort(as.character(unique(base_df$y)))
...
if (!is.character(mut_start)) {
  ...
  mut_bases <- base_df %>%
    ...
    count(base) %>%
    arrange(desc(n)) %>%  # Sort by frequency (descending, more frequent last)
    pull(base)
  ...
  if (unique_bases_count >= 2) {
    ...
    # Get y positions ordered by mut_bases frequency
    ordered_y <- base_df %>%
      filter(visual_x == mut_start - start_axis) %>%
      arrange(match(base, mut_bases)) %>%
      pull(y)
    ...
    base_df <- base_df %>%
      mutate(
        fill_color = ifelse(visual_x == mut_start - start_axis, config$visual$colors$base[base], fill_color)
        # y = factor(y, levels = ordered_y)
      )
  } else {
    # If only one base type, just modify color without reordering
    # base_df <- base_df %>%
    #   mutate(
    #     fill_color = ifelse(visual_x == mut_start, config$visual$colors$base[base], fill_color)
    #   )
    # base_df$y <- factor(base_df$y, levels = sort(unique(base_df$y)))
...
# base_df$y <- factor(as.character(base_df$y), levels =as.character(ordered_y))
# base_df$y <- factor(base_df$y, levels = sort(unique(base_df$y)))
...
base_plot <- ggplot(data = base_df, aes(x = visual_x, y = as.character(y))) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = config$visual$font$size*.pt*0.6, angle = 90, vjust = 0.5),
    axis.title.x = element_text(size = config$visual$font$size*.pt*1.5),
    axis.ticks.x = element_line(linewidth = 0.1 * .pt ),
...
adjusted_width = (range_x[2] - range_x[1] + 1)*0.5  + y_axis_width + legend_gap_width + guide_width

adjusted_height = (range_y[2] - range_y[1] + 1)/8 + x_axis_height
