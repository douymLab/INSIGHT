...
if (length(args) < 3) {
  stop("Usage: Rscript script.R <base_df_path> <read_df_path> <reference_df_path> [output_file] [mut_start] [mut_end] [mut_type]")
}
...
output_file <- if (length(args) >= 4) args[4] else "output.png"
mut_start <- if (length(args) >= 5) args[5] else "None"
mut_end <- if (length(args) >= 6) args[6] else "None"
mut_type <- if (length(args) >= 7) args[7] else "None"

plot_alignment <- function(base_df_path, read_df_path, reference_df_path, output_file,  mut_start, mut_end, mut_type) {
  # 处理可能为 None 的参数
  ...
  # max base_plot width
  base_plot_width <- diff(range_x) * base_width 
  
  # max base_plot height
  min_base_plot_height = 12 * base_height
  base_plot_height <- max(diff(range_y) * base_height, min_base_plot_height)
  
  # compute total size
  feature_all_width = feature_width * length(plot_list) * base_width
  sub_width = c(feature_all_width, base_plot_width, label_width)
  percent_width = prop.table(sub_width)
  total_width = margin * 2 + base_plot_width + feature_all_width + label_width + legend_with
  total_height = margin * 2 + base_plot_height
  
  units = 'in'
  
  proption_labels = percent_width[3]
  proption_feature = percent_width[1] / length(plot_list) * 1.2
  
  proption_ref = base_height / diff(range_y) * 2
  ...
  ggsave(output_file, p, width = total_width/5, height = ifelse(total_height/5 > 4, total_height/5,4), 
         units = units,
         dpi = 300, limitsize = FALSE)
}

# Call the function with command line arguments
plot_alignment(base_df_path, read_df_path, reference_df_path, output_file, mut_start, mut_end, mut_type)